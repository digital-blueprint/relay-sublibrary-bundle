        ...
        $treeBuilder->getRootNode()
            ->children()
                ...
                ->scalarNode('analytics_api_key')
                    ->info('The API key for the analytics API (defaults to api_key)')
                    ->example('your_key')
                ->end()
                ->booleanNode('readonly')
                    ...
                    ->defaultFalse()
                ->end()
                ->arrayNode('person_local_data_attributes')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('email')
                            ->info('The person local data attribute to request for the person email address')
                            ->defaultValue('email')
                        ->end()
                        ->scalarNode('alma_id')
                            ->info('The person local data attribute to request for the person ALMA ID')
                            ->defaultValue('almaId')
                        ->end()
                    ->end()
                ->end()
                ->arrayNode('organization_local_data_attributes')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('code')
